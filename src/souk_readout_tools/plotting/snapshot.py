...
import numpy as np
from ._common import _get_pyplot, _compute_mag_phase, ERRORBAR_STYLE
...
def plot_snapshots(snapshot_data, format='iq_vs_t', repetitions='concatenate',
    ...
    if format == 'iq':
        ...
        for z, label in zip(z_list, labels):
            ax.plot(z.real, z.imag, '.', markersize=1, label=label, **kwargs)
        ax.set_xlabel('I')
        ax.set_ylabel('Q')
        ax.set_aspect('equal')
        if repetitions == 'overlay' and n_snap <= 20:
    ...
    for z, label in zip(z_list, labels):
        ...
        if format == 'iq_vs_t':
            ax1.plot(t, z.real, linewidth=0.5, label=label, **kwargs)
            ax2.plot(t, z.imag, linewidth=0.5, label=label, **kwargs)
            ax1.set_ylabel('I')
            ax2.set_ylabel('Q')
            ...
            ax1.plot(t, mag_db, linewidth=0.5, label=label, **kwargs)
            ax2.plot(t, phase, linewidth=0.5, label=label, **kwargs)
...
def plot_snapshots_psd(snapshot_data, method='averaged', psd_kwargs=None,
                       show_errors=True, fig=None, **kwargs):
    """
    ...
    if method == 'averaged':
        ...
        if show_errors and psd_std is not None:
            ...
            ax.errorbar(f, psd_db, yerr=e_db, linewidth=0.5,
                        **{**ERRORBAR_STYLE, **kwargs})
        else:
            ax.plot(f, psd_db, linewidth=0.5, **kwargs)
...
def plot_batch_snapshots(batch_data, format='iq_vs_t', repetitions='concatenate',
    ...
    if format == 'iq':
        ...
        for row, (tidx, snap) in enumerate(sorted(results.items())):
            ...
            ax.plot(z.real, z.imag, '.', markersize=1, **kwargs)
            ax.set_aspect('equal')
            ...
            if format == 'iq_vs_t':
                if repetitions == 'overlay':
                    for s in range(n_snap):
                        ...
                        ax1.plot(ts, snapshots[s].real, linewidth=0.3, alpha=0.5)
                        ax2.plot(ts, snapshots[s].imag, linewidth=0.3, alpha=0.5)
                else:
                    ax1.plot(t, z.real, linewidth=0.5, **kwargs)
                    ax2.plot(t, z.imag, linewidth=0.5, **kwargs)
                ax1.set_ylabel('I')
                ax2.set_ylabel('Q')
                ...
                if repetitions == 'overlay':
                    for s in range(n_snap):
                        ...
                        ts = np.arange(n_samp) / sample_rate * 1e6
                        ax1.plot(ts, mag_db, linewidth=0.3, alpha=0.5)
                        ax2.plot(ts, phase, linewidth=0.3, alpha=0.5)
                else:
                    mag_db, phase = _compute_mag_phase(z)
                    ax1.plot(t, mag_db, linewidth=0.5, **kwargs)
                    ax2.plot(t, phase, linewidth=0.5, **kwargs)
                ax1.set_ylabel('|S21| (dB)')
                ax2.set_ylabel('Phase (rad)')

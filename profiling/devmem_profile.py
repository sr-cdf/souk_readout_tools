...
firmware_lib does on the fast interface:
  ...
  - accumulator READ: slice-read bytes out of the mmap, then np.frombuffer:
    ...
    (see read_accumulated_data_fast, firmware_lib.py:7935-7936)
...
def run(dev_path, n_steps, n_tones, n_read_chans, warmup, out_path,
    ...
    read_bytes = n_read_chans * 2 * 4   # real+imag, 4 bytes each
    ...
    write_times = []   # seconds per control-buffer write
    read_times = []    # seconds per accumulator read
    step_times = []    # seconds per full step (write + read)

    def one_step():
        t0 = time.perf_counter()
        mm[write_addr:write_addr + write_bytes] = payload.tobytes()
        t1 = time.perf_counter()
        raw = mm[read_addr:read_addr + read_bytes]
        dout = np.frombuffer(raw, dtype='<i4')
        t2 = time.perf_counter()
        # touch dout so the read isn't optimised away
        if dout.size and dout[0] == 0xDEADBEEF:
    ...
    s_write = summarize("write", write_times, write_bytes)
    ...
    with open(out_path, "w") as f:
        ...
        for s in (s_write, s_read, s_step):
        ...
        f.write("# columns: step_index  write_us  read_us  step_us\n")
        for i, (w, r, s) in enumerate(zip(write_times, read_times, step_times)):
            f.write(f"{i}\t{us(w):.4f}\t{us(r):.4f}\t{us(s):.4f}\n")
...
def main():
    ...
    p.add_argument("--read-chans", type=int, default=DEFAULT_N_TONES,
                   help="accumulator channels read back per step "
                        f"(default {DEFAULT_N_TONES})")
    ...
    run(args.dev, args.steps, args.tones, args.read_chans, args.warmup,
        args.out, args.write_addr, args.read_addr)

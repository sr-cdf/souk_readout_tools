...
Copy both config files to the RFSoC:
...
You can start each server instance with a different config file.

#### Terminal 1 (pipeline 0)
```bash
ssh casper@rfsoc
sudo /home/casper/py38venv/bin/souk-readout-server ~/.souk_readout_tools/config/config_pipeline_0.yaml
```

#### Terminal 2 (pipeline 1)
```bash
ssh casper@rfsoc
sudo /home/casper/py38venv/bin/souk-readout-server ~/.souk_readout_tools/config/config_pipeline_1.yaml
```
...
server = ReadoutServer(config_file="/home/casper/.souk_readout_tools/config/config_pipeline_0.yaml")
...
On your local machine (or wherever you run the clients):

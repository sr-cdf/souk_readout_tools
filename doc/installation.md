...
- **Clock configuration:** The PL clocks (LMK/LMX chips) are configured on boot by the `krc-utils` service. The default source is the on-board 12.8 MHz oscillator. To use an external 10 MHz reference, set `firmware.clock_source: "external"` in the config; it is applied safely at the next (re)program. Before firmware access the server runs a pure, PS-side clock check (it never runs `krc-utils init` as a gate): if a PLL is unlocked, or the live source differs from the config, it refuses to touch the firmware and reports `reset_required` — recover with a `hard_reset`, which applies the change via a reset-to-base → clock-init → reprogram cycle (resets both pipelines). `krc-utils init` is only ever run once the PL is reset to zero tones (issue #14). See [clock_source.md](clock_source.md) for details.
